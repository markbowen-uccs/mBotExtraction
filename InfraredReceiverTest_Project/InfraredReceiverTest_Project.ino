...
void setup()
{
  ...
  pinMode(A7, INPUT); //Center Sensor
  pinMode(A8, INPUT); //Right Sensor
}

void loop()
{

  int LEFT_SENSOR = digitalRead(A6);
  int CENTER_SENSOR = digitalRead(A7);
  int RIGHT_SENSOR = analogRead(A8);

  Serial.println(String(LEFT_SENSOR) + " " + String(CENTER_SENSOR) + " " + String(RIGHT_SENSOR));
